...
function aghanim_spell_swap:OnAbilityPhaseStart()
	if IsServer() then
		...
		self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_black_king_bar_immune", {duration = self:GetChannelTime() + 1.25})	
	end
	return true
end
...
function aghanim_spell_swap:OnChannelFinish( bInterrupted )
	if IsServer() then
		...
		for k,hHero in pairs ( self.Heroes ) do
			if hHero ~= nil and hHero.IsRealHero ~= nil and hHero:IsRealHero() then
...
function aghanim_spell_swap:OnSpellStart()
	if IsServer() then
		...
		for k,hHero in pairs ( self.Heroes ) do
			if NotNull(hHero) and hHero.IsRealHero ~= nil and hHero:IsRealHero() then
