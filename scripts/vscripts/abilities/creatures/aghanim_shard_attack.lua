	...
	if IsServer() then
		for _, v in pairs ( self.Projectiles ) do
			if v.nFXIndex then
				...
				ParticleManager:ReleaseParticleIndex(v.nFXIndex)
			end

			if IsNull(v.attachEnt) == false then
				UTIL_Remove(v.attachEnt)
			end
		end
...
function aghanim_shard_attack:ThinkSpiralProjectile( info )
	...
	if IsNull(info.attachEnt) == false then
...
function aghanim_shard_attack:OnProjectileHitHandle( hTarget, vLocation, iProjectileHandle )
	if IsServer() then
		...
		if hTarget ~= nil and hTarget:IsNull() == false and not hTarget:IsMagicImmune() and not hTarget:IsInvulnerable() then
			local damage = 
			{
				victim = hTarget,
