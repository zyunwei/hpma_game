...
调用方式
AnimationTimer([unique_str,] duration, callback)
AnimationTimer([unique_str,] entity, duration, callback)

callback说明：
callback( factor, time )
...
function AnimationTimer( ... )
    local arg1,arg2,arg3,arg4 = ...
	local entity,duration,callback,unique_str
	...
	if type(entity) ~= "table" or type(duration) ~= "number" or type(callback) ~= "function" then
		error('arguments is wrong', 2)
	end

	local fStartTime = GameRules:GetGameTime()
	entity:SetContextThink(unique_str, function ()
		if entity:IsNull() then
			return nil
		end
        ...
        local time = GameRules:GetGameTime() - fStartTime
        callback( math.min(time/duration, 1.0), math.min(time, duration) )
