...
function __AvalonEvents__:__OnPlayerLevelUp( keys )
    ...
    if player == nil or hero == nil then return end
...
function __AvalonEvents__:__OnPlayerConnectFull( keys )
    if keys == nil or keys.PlayerID == nil then return end
    GameEvents:OnPlayerConnectFull(keys.PlayerID, PlayerResource:GetPlayer(keys.PlayerID))
end
...
function __AvalonEvents__:__OnPlayerChat( keys )
    ...
    if IsInToolsMode() or GameRules.XW:IsDeveloper(keys.playerid) then
...
function __AvalonEvents__:__OnPickHero(keys)
    GameEvents:OnPickHero(keys.player, keys.heroindex)
end
...
if __AvalonEventsInit__ == nil then
    ...
    ListenToGameEvent("dota_player_pick_hero", Dynamic_Wrap(__AvalonEvents__, "__OnPickHero"), __AvalonEvents__)
    ...
    GameRules:GetGameModeEntity():SetModifyGoldFilter(Dynamic_Wrap(Filter, "ModifyGoldFilter"),Filter)
end
