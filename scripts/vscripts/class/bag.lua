...
function public:Update()
	...
	for i, v in ipairs(self.__itemlist) do
		local info = { entindex = v, charges = 0, stackable = false }
		local item = EntIndexToHScript(v)
		if NotNull(item) then
			info.charges = item:GetCurrentCharges()
			info.stackable = item:IsStackable()
		end
		table.insert(items, info)
	end

	CustomNetTables:SetTableValue("Bag", self.__nettable_name, items)
...
function public:RemoveItem(item)
	...
	if bagSlot > 0 then
		...
		item:RemoveSelf()
		itemlist[bagSlot] = -1
		self:NotifySlotItemChanged(bagSlot)
		self:Update()
		return true
	end
...
function public:NotifySlotItemChanged(slotIndex)
	local owner = self:GetOwner()
	if IsNull(owner) == false and owner:GetPlayerOwner() then
		CustomGameEventManager:Send_ServerToPlayer(owner:GetPlayerOwner(), "item_swap_alert_remove", { slot_index = slotIndex })
	end
end
...
function public:SwapItem(slot1,slot2)
	...
	self:NotifySlotItemChanged(slot1)
	self:NotifySlotItemChanged(slot2)
	self:Update()
...
function public:Discard(bagSlot)
	...
	if itemIndex > 0 then
		...
		if IsNull(item) == false then
			local drop = CreateItemOnPositionSync(origin, EntIndexToHScript(itemIndex))
			local itemConfig = ItemConfig[item:GetName()]
...
function public:Merge(bagSlot1, bagSlot2)
	...
	itemlist[bagSlot2] = -1
	item2:RemoveSelf()
	self:NotifySlotItemChanged(bagSlot1)
	self:NotifySlotItemChanged(bagSlot2)
...
function public:GetItemSlotIndex(item)
	local min, max = self:GetMin(), self:GetMax()
	for bagSlot = min, max do
		local itemIndex = self.__itemlist[bagSlot]
		if itemIndex and itemIndex > 0 then
			if self:GetItem(bagSlot) == item then
				return bagSlot
			end
		end
	end
...
function public:GetAllItems()
	...
	for bagSlot = min, max do
		...
		if itemIndex and itemIndex > 0 then
			local item = EntIndexToHScript(itemIndex)
			if item and not item:IsNull() then
				table.insert(itemList, item:GetAbilityName())
			end
		end
	end
	return itemList
