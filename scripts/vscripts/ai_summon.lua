...
if SummonAI == nil then SummonAI = class({}) end
...
SummonAI.CastToFarestPointAbility = {
...
SummonAI.CheckModifierPointTargetAbility = {
    "doom_bringer_doom",
    "bloodseeker_rupture",
...
SummonAI.CastToNearestPointAbility = {
    "nyx_assassin_impale",
    "lion_impale",
    "zuus_lightning_bolt",
...
function SummonAI:IsValidPosition(pos)
...
function SummonAI:GetAdjustPosition(pos)
...
function SummonAI:IsAlive(target)
...
function SummonAI:IsValidTargetToCast(target)
    ...
    if(SummonAI:IsValidPosition(target:GetAbsOrigin()) == false) then
        return false
    end
...
function SummonAI:IsTaunt(hero)
...
function SummonAI:HasTargetTrueSight(hero, target)
...
function SummonAI:OnHeroThink(hero)
    ...
    if(SummonAI:IsAlive(hero) == false and hero:IsReincarnating() == false) then
        return nil
    end
    ...
    if(SummonAI:IsValidPosition(heroPosition) == false) then
        local newPos = SummonAI:GetAdjustPosition(heroPosition)
    ...
    for i, v in pairs(SUMMON_CMD_LIST) do
        local score, cmdData = SummonAI:EvaluateCommand(hero, v)
    ...
    if(highestData ~= nil and highestScore > 0) then
        local delay = SummonAI:ExecuteCommand(hero, SUMMON_CMD_LIST[highestScoreCommand], highestData)
...
function SummonAI:IsValidSummonAttackTarget(hero, target)
    local owner = hero:GetOwner()
    if IsNull(owner) == false then
        if (owner:GetAbsOrigin() - target:GetAbsOrigin()):Length2D() > SummonAI.CONTROL_RADIUS then
            return false
        end
    end
...
function SummonAI:EvaluateCommand(hero, cmdName)
    if(hero == nil or hero:IsNull()) then
        return 0, nil
    end
    ...
    if(cmdName == "ATTACK_TARGET") then
        ...
        if(attackTarget == nil or SummonAI:IsAlive(attackTarget) == false) then
            local closestTarget = SummonAI:ClosestEnemyAll(hero, hero:GetTeamNumber())
            if(closestTarget == nil or SummonAI:IsAlive(closestTarget) == false) then
                return 0, nil
            end
    ...
    if(cmdName == "USE_ABILITY") then
        ...
        for i = 0, hero:GetAbilityCount() - 1 do
            ...
            if canCast and table.contains(SummonAI.DontCastAbility, ability:GetName()) == false then
        ...
        if(#canCastAbilities > 0) then
            for _, v in pairs(canCastAbilities) do
                if(v:IsToggle() and v:GetToggleState() == false and table.contains(SummonAI.ToggleOnAbility, v:GetName())) then
                    v:ToggleAbility()
                end
            end
        end
        ...
        for _, v in pairs(canCastAbilities) do
            ...
            if(table.contains(SummonAI.PriorityCastNoTargetAbility, abilityName) == false) then
                local spellData = SummonAI:GetSpellData(v)
            ...
            if(slotIndex <= 5 or slotIndex == 15 or slotIndex == 16) then
                ...
                if(item ~= nil) then
                    ...
                    elseif(item:IsInAbilityPhase()) then
                        canCast = false
                    elseif(table.contains(SummonAI.DontCastItems, itemName)) then
                        canCast = false
                    end
    ...
    if(cmdName == "MOVE_TO_POSITION") then
        ...
        if IsNull(owner) == false then
            local ownerAttackTarget = owner:GetAttackTarget()
            if SummonAI:IsAlive(ownerAttackTarget) ~= false then
                return 0, nil
            end

            local length = (owner:GetAbsOrigin() - hero:GetAbsOrigin()):Length2D()
            if length > SummonAI.CONTROL_RADIUS then
                if length > 2000 then
                ...
                if(hero:GetName() == "npc_dota_hero_batrider") then
                    ...
                    elseif(hero:HasModifier("modifier_batrider_firefly")) then
                        local enemy = SummonAI:GetFarestEnemyTarget(hero, 800)
                        if(enemy ~= nil) then
                            local vTargetLoc = enemy:GetAbsOrigin()
                            if SummonAI:IsValidPosition(vTargetLoc) and (hero:GetAbsOrigin() - vTargetLoc):Length2D() > 100 then
                                return 4, vTargetLoc
                            end
                        end
                    end
...
function SummonAI:ExecuteCommand(hero, cmdName, cmdData)
    if(hero == nil or hero:IsNull()) then
        return 0.2
    end
    ...
    if(cmdName == "ATTACK_TARGET") then
        ...
        if(SummonAI:IsValidPosition(targetPosition) == false) then
    ...
    if(cmdName == "USE_ABILITY") then
        ...
        local loopTime = SummonAI:CastSpell(cmdData)
        return loopTime
    end
    ...
    if(cmdName == "MOVE_TO_POSITION") then
        if(SummonAI:IsValidPosition(cmdData) == false) then
            hero:MoveToPositionAggressive(hero:GetAbsOrigin())
            return 0.2
        end
...
function SummonAI:CastSpell(spellData)
    ...
    if(SummonAI:IsAlive(caster) == false) then
        return 0.2
    end
    ...
    if(spellData.type == "unit_target") then
        return SummonAI:CastSpellUnitTarget(hSpell, spellData.target)
    end
    
    if(spellData.type == "point_target") then
        return SummonAI:CastSpellPointTarget(hSpell, spellData.target)
    end
    
    if(spellData.type == "no_target") then
        return SummonAI:CastSpellNoTarget(hSpell)
    end
    
    if(spellData.type == "tree_target") then
        return SummonAI:CastSpellTreeTarget(hSpell, spellData.target)
    end
    
    return 0.2
end

function SummonAI:HasInvisibleEnemyNearby(hero, range)
    if(range == nil) then
        range = 850
    end
...
function SummonAI:GetSpellData(hSpell)
    ...
    if(abilityName == "item_lotus_orb") then
        local hTarget = SummonAI:GetNearestFriendWithoutBuff(hSpell)
        if hTarget ~= nil and SummonAI:IsAlive(hTarget) and hTarget ~= hero then
    ...
    if(abilityName == "magnataur_empower" or abilityName == "ogre_magi_bloodlust" or abilityName == "treant_living_armor" or abilityName == "tinker_defense_matrix") then
        local hTarget = SummonAI:GetNearestFriendWithoutBuff(hSpell)
        if(hTarget ~= nil and SummonAI:IsAlive(hTarget)) then
            return {ability = hSpell, type = "unit_target", target = hTarget}
        end
    ...
    if(abilityName == "witch_doctor_death_ward") then
        local castRange = SummonAI:GetSpellRange(hSpell)
        ...
        if(target ~= nil) then
            ...
            if SummonAI:IsValidPosition(castPos) then
    ...
    if(abilityName == "keeper_of_the_light_chakra_magic") then
        local hTarget = SummonAI:GetLowManaFriendlyTarget(hSpell, 101)
        if(hTarget ~= nil and SummonAI:IsAlive(hTarget)) then
            return {ability = hSpell, type = "unit_target", target = hTarget}
        end
        return nil
    ...
    if(table.contains(SummonAI.CastToFarestPointAbility, abilityName)) then
        local hTarget = SummonAI:GetFarestEnemyTarget(hero, (SummonAI:GetSpellRange(hSpell)) * 0.8)
        if(hTarget ~= nil and SummonAI:IsAlive(hTarget)) then
            ...
            if(SummonAI:IsValidPosition(castLocation)) then
    ...
    if(table.contains(SummonAI.CastToNearestPointAbility, abilityName)) then
        local hTarget = SummonAI:GetClosestEnemyTarget(hero, SummonAI:GetSpellRange(hSpell))
        if(hTarget ~= nil and SummonAI:IsAlive(hTarget)) then
    ...
    if(abilityName == "storm_spirit_ball_lightning") then
        ...
        local hTarget = SummonAI:GetFarestEnemyTarget(hero, 1200)
        if(hTarget ~= nil and SummonAI:IsAlive(hTarget)) then
            local castLength = (hTarget:GetAbsOrigin() - hero:GetAbsOrigin()):Length2D()
            local castVector = (hTarget:GetAbsOrigin() - hero:GetAbsOrigin()):Normalized()
    ...
    if(abilityName == "pudge_meat_hook" or abilityName == "pet_pudge_meat_hook") then
        local hTarget = SummonAI:GetBestTargetInRange(hSpell)
        if(hTarget ~= nil and SummonAI:IsAlive(hTarget)) then
    ...
    if(abilityName == "oracle_false_promise" or abilityName == "winter_wyvern_cold_embrace" or abilityName == "dazzle_shallow_grave") then
        local hTarget = SummonAI:GetBestFriendlyTarget(hSpell, 0.6)
        if(hTarget ~= nil and SummonAI:IsAlive(hTarget) and SummonAI:CheckTargetNoModifier(hSpell, hTarget) == true) then
            return {ability = hSpell, type = "unit_target", target = hTarget}
        end
    ...
    if(abilityName == "mars_spear") then
        local hTarget = SummonAI:FindEnemyByModifier(hSpell, "modifier_eul_cyclone")
        if(hTarget ~= nil and SummonAI:IsAlive(hTarget)) then
    ...
    if(abilityName == "ursa_earthshock") then
        ...
        if(SummonAI:HasEnemyNearPosition(hero, checkPoint, 300)) then
            return {ability = hSpell, type = "no_target", target = nil}
        end
        return nil
    end
    ...
    if(abilityName == "weaver_shukuchi") then
        local heroPos = hero:GetAbsOrigin()
        if(hero:GetHealthPercent() <= 50 or SummonAI:HasEnemyNearby(hero, 550)) then
            return {ability = hSpell, type = "no_target", target = nil}
        end
        return nil
    end
    ...
    if(abilityName == "weaver_time_lapse") then
        if(hero:HasScepter()) then
            local hTarget = SummonAI:GetBestFriendlyTarget(hSpell, 0.5)
            if(hTarget ~= nil and SummonAI:IsAlive(hTarget)) then
                return {ability = hSpell, type = "unit_target", target = hTarget}
            end
        else
    ...
    if(abilityName == "kunkka_ghostship") then
        local vTargetLoc = SummonAI:GetBestAOEPointTarget(hSpell, DOTA_UNIT_TARGET_TEAM_ENEMY)
        if SummonAI:IsValidPosition(vTargetLoc) then
    ...
    if bitContains(nTargetType, DOTA_UNIT_TARGET_TREE) then
        local treeTarget = SummonAI:FindTreeTarget(hSpell)
    ...
    elseif bitContains(nTargetTeam, DOTA_UNIT_TARGET_TEAM_CUSTOM) then
        if bitContains(nTargetFlags, DOTA_UNIT_TARGET_FLAG_NOT_CREEP_HERO) then
            local hTarget = SummonAI:GetBestCreepTarget(hSpell)
            if hTarget ~= nil and SummonAI:IsAlive(hTarget) then
                return {ability = hSpell, type = "unit_target", target = hTarget}
            end
        else
    ...
    elseif bitContains(nTargetTeam, DOTA_UNIT_TARGET_TEAM_ENEMY) then
        if bitContains(nBehavior, DOTA_ABILITY_BEHAVIOR_NO_TARGET) then
            if SummonAI:IsNoTargetSpellCastValid(hSpell, DOTA_UNIT_TARGET_TEAM_ENEMY) then
                return {ability = hSpell, type = "no_target", target = nil}
            end
        elseif bitContains(nBehavior, DOTA_ABILITY_BEHAVIOR_DIRECTIONAL) then
            local vTargetLoc = SummonAI:GetBestDirectionalPointTarget(hSpell, DOTA_UNIT_TARGET_TEAM_ENEMY)
            if SummonAI:IsValidPosition(vTargetLoc) then
                return {ability = hSpell, type = "point_target", target = vTargetLoc}
            end
    ...
    elseif bitContains(nTargetTeam, DOTA_UNIT_TARGET_TEAM_FRIENDLY) then
        if bitContains(nBehavior, DOTA_ABILITY_BEHAVIOR_NO_TARGET) then
            if SummonAI:IsNoTargetSpellCastValid(hSpell, DOTA_UNIT_TARGET_TEAM_FRIENDLY) then
                return {ability = hSpell, type = "no_target", target = nil}
            end
        elseif bitContains(nBehavior, DOTA_ABILITY_BEHAVIOR_POINT) then
            local vTargetLoc = SummonAI:GetBestAOEPointTarget(hSpell, DOTA_UNIT_TARGET_TEAM_FRIENDLY)
            if SummonAI:IsValidPosition(vTargetLoc) then
                return {ability = hSpell, type = "point_target", target = vTargetLoc}
            end
        else
            local hTarget = SummonAI:GetBestFriendlyTarget(hSpell, 1.0)
            if hTarget ~= nil and SummonAI:IsAlive(hTarget) then
                return {ability = hSpell, type = "unit_target", target = hTarget}
            end
        end
...
function SummonAI:ClosestEnemyAll(hero, teamId)
    if(hero == nil or hero:IsNull()) then
        return nil
    end

    local enemies = FindUnitsInRadius(teamId, hero:GetAbsOrigin(), nil, SummonAI.CONTROL_RADIUS, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_NO_INVIS, FIND_CLOSEST, true)
    ...
    for index = 1, #enemies do
        if(enemies[index]:IsInvisible() == false or SummonAI:HasTargetTrueSight(hero, enemies[index])) then
            if(SummonAI:IsValidPosition(enemies[index]:GetAbsOrigin()) and SummonAI:IsAlive(enemies[index]) and enemies[index]:IsInvulnerable() == false and enemies[index]:IsAttackImmune() == false) then
...
function SummonAI:GetSpellRange(hSpell)
    if(hSpell == nil) then
        return 250
    end
...
function SummonAI:GetBestTargetInRange(hSpell, findFarthest)
    ...
    local radius = SummonAI:GetSpellRange(hSpell)

    local needCheckModifier = table.contains(SummonAI.CheckModifierPointTargetAbility, abilityName)
    ...
    for index = 1, #enemies do
        if(enemies[index]:IsInvisible() == false or SummonAI:HasTargetTrueSight(hero, enemies[index])) then
            if(SummonAI:IsAlive(enemies[index]) and SummonAI:IsValidTargetToCast(enemies[index])) then
                if(enemies[index]:IsMagicImmune() == false or castMagicImmuneTarget) then
                    if(needCheckModifier == false or SummonAI:CheckTargetNoModifier(hSpell, enemies[index]) == true) then
                        firstEnemy = enemies[index]
                        break
                    end
                end
...
function SummonAI:CheckTargetNoModifier(hSpell, targetHero)
...
function SummonAI:GetBestFriendlyTarget(hSpell, minHpPercent)
    ...
    SummonAI:GetSpellRange(hSpell), DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO, hSpell:GetAbilityTargetFlags(), 0, true)
    ...
    for _, v in pairs(friends) do
        if(SummonAI:IsAlive(v) and SummonAI:IsValidTargetToCast(v)) then
...
function SummonAI:GetNearestFriendWithoutBuff(hSpell)
    local hero = hSpell:GetCaster()

    local friends = FindUnitsInRadius(hero:GetTeamNumber(), hero:GetAbsOrigin(), hero,
    SummonAI:GetSpellRange(hSpell), DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO, hSpell:GetAbilityTargetFlags(), FIND_CLOSEST, true)
    ...
    local target = nil
    for _, v in pairs(friends) do
        if v ~= hero and SummonAI:IsAlive(v) and SummonAI:IsValidTargetToCast(v) then
            if(SummonAI:CheckTargetNoModifier(hSpell, v) == true) then
                target = v
                break
            end
        end
    ...
    if(target == nil) then
        if(SummonAI:CheckTargetNoModifier(hSpell, hero) == true) then
            return hero
        else
            return nil
        end
...
function SummonAI:GetNearestFriendWithBuff(hero, range, modifierName)
    local friends = FindUnitsInRadius(hero:GetTeamNumber(), hero:GetAbsOrigin(), hero,
...
function SummonAI:GetBestCreepTarget(hSpell)
    local enemies = FindUnitsInRadius(hSpell:GetCaster():GetTeamNumber(), hSpell:GetCaster():GetAbsOrigin(), hSpell:GetCaster(),
    SummonAI:GetSpellRange(hSpell), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_CREEP, hSpell:GetAbilityTargetFlags(), FIND_CLOSEST, true)
    ...
    for _, v in pairs(enemies) do
        if(v:IsInvisible() == false or SummonAI:HasTargetTrueSight(hSpell:GetCaster(), v)) then
            if(SummonAI:IsValidPosition(v:GetAbsOrigin()) and SummonAI:IsAlive(v)) then
...
function SummonAI:GetNotImmueCreepTarget(hSpell)
    local enemies = FindUnitsInRadius(hSpell:GetCaster():GetTeamNumber(), hSpell:GetCaster():GetAbsOrigin(), hSpell:GetCaster(),
    SummonAI:GetSpellRange(hSpell), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_CREEP, SUMMON_UNIT_FILTER, FIND_CLOSEST, true)
    ...
    for _, v in pairs(enemies) do
        if(v:IsInvisible() == false or SummonAI:HasTargetTrueSight(hSpell:GetCaster(), v)) then
            if(SummonAI:IsAlive(v) and v:IsMagicImmune() == false and v:IsInvulnerable() == false) then
                return v
            end
        end
    end
...
function SummonAI:GetSpellCastTime(hSpell)
...
function SummonAI:FindTreeTarget(hSpell)
    local Trees = GridNav:GetAllTreesAroundPoint(hSpell:GetCaster():GetAbsOrigin(), SummonAI:GetSpellRange(hSpell), false)
    if #Trees == 0 then
        return nil
    end
...
function SummonAI:CastSpellNoTarget(hSpell)
    local caster = hSpell:GetCaster()
    if(caster == nil or caster:IsNull() or SummonAI:IsAlive(caster) == false) then
        return 0.2
    end
    ...
    return SummonAI:GetSpellCastTime(hSpell)
end

function SummonAI:CastSpellUnitTarget(hSpell, hTarget)
    local caster = hSpell:GetCaster()
    if(caster == nil or caster:IsNull() or SummonAI:IsAlive(caster) == false) then
        return 0.2
    end
    
    if(hTarget == nil or hTarget:IsNull() or SummonAI:IsAlive(hTarget) == false) then
        return 0.2
    end
    ...
    if(SummonAI:IsValidPosition(hTarget:GetAbsOrigin()) == false) then
        return 0.2
    end
...
function SummonAI:CastSpellTreeTarget(hSpell, treeTarget)
    local caster = hSpell:GetCaster()
    if(caster == nil or caster:IsNull() or SummonAI:IsAlive(caster) == false) then
        return 0.2
    end
...
function SummonAI:CastSpellPointTarget(hSpell, vLocation)
    local caster = hSpell:GetCaster()
    if(caster == nil or caster:IsNull() or SummonAI:IsAlive(caster) == false) then
        return 0.2
    end
    
    if(SummonAI:IsValidPosition(vLocation) == false) then
        return 0.2
    end
...
function SummonAI:IsNoTargetSpellCastValid(hSpell, targetTeamType)
...
function SummonAI:GetBestAOEPointTarget(hSpell, targetTeamType)
    if(hSpell == nil) then
        return nil
    end
    local caster = hSpell:GetCaster()
    ...
    local searchRadius = SummonAI:GetSpellRange(hSpell) + nAbilityRadius

    local ignoreMagicImmune = false

    local vLocation = SummonAI:GetBestAOELocation(hSpell, searchRadius, nUnitsRequired, ignoreMagicImmune, targetTeamType, nil)

    return vLocation
end

function SummonAI:GetBestAOELocation(hSpell, searchRadius, unitsRequired, ignoreMagicImmune, targetTeamType, searchStartPoint)
    local hero = hSpell:GetCaster()
    if(hero == nil or hero:IsNull()) then
        return nil
    end
    ...
    for _, v in pairs(targets) do
        if(v:IsInvisible() == false or SummonAI:HasTargetTrueSight(hero, v)) then
            if(SummonAI:IsAlive(v) and SummonAI:IsValidTargetToCast(v)) then
    ...
    for _, g in pairs(checkPointsGroup) do
        ...
        if SummonAI:IsValidPosition(point1) then
...
function SummonAI:GetBestDirectionalPointTarget(hSpell, targetTeamType)
    if(hSpell == nil) then
        return nil
    end
    local caster = hSpell:GetCaster()
...
function SummonAI:GetBestLinearTarget(hSpell, width)
    ...
    local vLocation = GetTargetLinearLocation(hSpell:GetCaster():GetTeamNumber(),
        ...
        hSpell:GetCaster():GetAbsOrigin(),
        SummonAI:GetSpellRange(hSpell) * 0.75,
        width,
    nUnitsRequired)
...
function SummonAI:GetClosestEnemyTarget(hero, radius)
    if(hero == nil or hero:IsNull()) then
        return nil
    end
...
function SummonAI:GetFarestEnemyTarget(hero, radius)
    if(hero == nil or hero:IsNull()) then
        return nil
    end
...
function SummonAI:GetLowManaFriendlyTarget(hSpell, manaPercent)
    ...
    SummonAI:GetSpellRange(hSpell), DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO, UNIT_FILTER, 0, true)
...
function SummonAI:FindEnemyByModifier(hSpell, modifierName)
    local hero = hSpell:GetCaster()
    if IsNull(hero) then
        return nil
    end
...
function SummonAI:HasEnemyNearPosition(hero, pos, range)
    if(range == nil) then
        range = 850
    end
    ...
    for _, v in pairs(units) do
        if SummonAI:IsValidTargetToCast(v) then
...
function SummonAI:HasEnemyNearby(hero, range)
    if(range == nil) then
        range = 850
    end
